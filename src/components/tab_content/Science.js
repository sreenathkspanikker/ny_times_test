...
import { scienceNewsAction, filterAction } from '../../redux';
import { useNavigate } from 'react-router-dom';
import { Pagination } from 'antd';

export const Science = ({ filter }) => {
    ...
    // get redux store
    const data = {
        scienceNews: useSelector(state => state.scienceNews?.data?.data),
    ...
    useEffect(() => {
        if (data?.filterData?.response?.docs.length > 0 && isFilter) {
            ...
            setListData(data?.scienceNews?.results)
        }
    }, [data, isFilter])
    ...
    // navigate to detail
    const handleClick = (e) => {
        navigate(`/details/:${`${`scienceNews`}-${e}`}`);
    }
